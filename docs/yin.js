...
const YIN = (() => {
  ...
  /* Returns {hz, confidence}; hz is 0 when unvoiced. `block` must be the
   * full analysis window (2048 samples). */
  function yin(block, sampleRate) {
  ...
  /* Frame-by-frame detection with gating and median smoothing, fed
   * hop-sized blocks. Mirrors PitchDetector in the Python version. */
  class Detector {
    constructor(sampleRate, { window = 2048, hop = 512,
    ...
    process(block) {
      ...
      return { hz: smoothed, confidence, levelDb };
    }
  }

  return { yin, rmsDb, Detector, MIN_HZ, MAX_HZ };
})();

if (typeof module !== "undefined") module.exports = YIN;   // node tests
